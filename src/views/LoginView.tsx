...
export const LoginView: React.FC = () => {
  ...
  const redirectTarget = activeView.type === "login" ? activeView.redirectView : undefined;
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotification("Welcome back to ILens Eyewear!", "success");
      if (redirectTarget) {
      ...
      if (err.code === "auth/invalid-credential" || err.code === "auth/user-not-found" || err.code === "auth/wrong-password") {
        ...
        setError("Failed to sign in. Please check your network and credentials.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-md mx-auto px-4 py-12 sm:py-16">
      <div className="bg-white p-8 sm:p-10 rounded-3xl border border-neutral-200 shadow-xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-black font-serif text-neutral-950">Welcome Back</h1>
          <p className="text-xs text-neutral-500">
            Sign in to access your prescriptions, order history, and ILens Circle points.
          </p>
        </div>
        ...
        <form onSubmit={handleEmailLogin} className="space-y-4 text-xs">
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center">
              ...
              <button
                type="button"
                onClick={() => setActiveView({ type: "forgot-password" })}
          ...
          >
            {loading ? "Signing In..." : "Sign In to ILens"}
        ...
        <div className="pt-2 text-center text-xs text-neutral-600">
          <span>Don't have an ILens account yet? </span>
          <button
            type="button"
            onClick={() => setActiveView({ type: "signup", redirectView: redirectTarget })}
